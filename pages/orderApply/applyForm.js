...
Page({
  ...
  fnComfirn() {
    ...
    // url="/pages/services/detail?order_no={{item.trade_no}}&sale_id={{item.id}}"
    wx.showLoading({
      title: '提交中...',
      mask: true
    })
    global.yhsd.sdk.service.save(oData, ({ res }) => {
      if (res.code === 200) {
        ...
        wx.hideLoading()
        wx.redirectTo({
          url: `/pages/services/detail?order_no=${this.data.order_no}&sale_id=${this.data.trade_after_sale_id}`
        })
      } else {
        wx.hideLoading()
